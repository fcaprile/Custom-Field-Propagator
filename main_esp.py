from front_end_ui_esp import Ui_MainWindow
from PyQt5.QtWidgets import QFileDialog
from PyQt5 import QtWidgets
from pyqtgraph.Qt import QtGui
from No_mask import *
from VPP import *
from plot_esp import *
from SPP import *
...
#        phip0=float(self.lineEdit_7.text())# por ahora no lo implemento, es el angulo 0 desde el que observas
    ...
    def save_parameters(self):
        try:
            ...
            if str(folder)=='(dejar así para usar carpeta local)':
                folder=''
            ...
            'n': self.parameters[1],
            'Radio de apertura': self.parameters[2]*10**-6,
            'Distancia focal (mm)': self.parameters[3]*10**-6,
            'Radio del haz gaussiano (mm)': self.parameters[4]*10**-6,
            'Longitud de onda en vacío (nm)': self.parameters[5]*self.parameters[3],
            'arctan(ey ex) (psi) (grados)': self.parameters[6],
            'Diferencia de fase entre ex y ey (beta) (grados)': self.parameters[7],
            'Distancia axial para el plano XY (z0) (nm)': self.parameters[8],
            'Resolución radial (nm)': self.parameters[9],
            'Resolución axial (nm)': self.parameters[10],
            'field of view radial (nm)': self.parameters[11],
            'field of view axial (nm)': self.parameters[15],
            'Intensidad del laser': self.parameters[12],
            'Radio de máscara de fase (mm)': self.parameters[13],
            'Distancia entre máscara de fase y lente objetivo (mm)': self.parameters[14]}
        ...
        try:                             
            if self.radioButton.isChecked()==False: #then no need to calculate the field at the entrance of the lens
                ...
                if selected==0: #VPP mask
                    ...
                    self.Ifield_xy=np.abs(ex2)**2+np.abs(ey2)**2+np.abs(ez2)**2
                if selected==1:
                    print('Propagación del campo incidente solo posible para máscaras VPP y elegida por el usuario')
                if selected==2: #Custom mask
                    # maskfunction=lambda theta, phi: np.exp(1j*phi) is the VPP mask function
                    #Usual parameters for integration precision:
